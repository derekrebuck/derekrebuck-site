...
function syncHeader() {
  ...
  const foreground = getComputedStyle(active).color || '#ffffff';
  const lightForeground = isLightBackground(foreground);
  header.style.setProperty('--header-bg', background);
  header.style.setProperty('--header-fg', foreground);
  header.dataset.headerTone = lightForeground ? 'dark' : 'light';

  if (logo) {
    const src = lightForeground ? logo.dataset.logoDark : logo.dataset.logoLight;
